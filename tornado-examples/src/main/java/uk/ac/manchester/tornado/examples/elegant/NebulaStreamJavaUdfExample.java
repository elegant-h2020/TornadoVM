 ...
 * </p>
 * <code>
 *     tornado --printKernel --threadInfo --jvm="-Ds0.t0.device=0:0 -Dtornado.recover.bailout=False -Dtornado.cim.mode=True" -m tornado.examples/uk.ac.manchester.tornado.examples.elegant.NebulaStreamJavaUdfExample 1024 tornado 100
 * </code>
 *
 */
public class NebulaStreamJavaUdfExample {
    ...
    public static PolarCoordinate map(final CartesianCoordinate inputmap) {
        ...
        output.radius = (float) Math.sqrt(inputmap.x * inputmap.x + inputmap.y * inputmap.y);
        output.angle = (float) Math.atan2(inputmap.x, inputmap.y);
        return output;
    }

    public static Float2 tornadoMap(final Float2 value) {
    ...
    public static void map(VectorFloat2 value, VectorFloat2 output) {
        for (@Parallel int i = 0; i < output.getLength(); i++) {
            output.set(i, tornadoMap(value.get(i)));
        }
    }

    public static boolean validate(PolarCoordinate[] resultJava, VectorFloat2 resultTornado) {
        boolean valid = true;
    ...
    public static void main(String[] args) {
        ...
        VectorFloat2 input = new VectorFloat2(size);
        ...
        for (int i = 0; i < size; i++) {
            ...
            input.set(i, new Float2(value, value));
        ...
        if (!executionType.equals("sequential")) {
            ...
            graph.transferToDevice(DataTransferMode.EVERY_EXECUTION, input) //
                    .task("t0", NebulaStreamJavaUdfExample::map, input, resultTornado) //
